...
/**
 * Deduplicate periods by start date + end date + description
 * This avoids duplicates when semester II has both terminal and non-terminal variants
 */
function deduplicatePeriods(periods: AcademicPeriod[]): AcademicPeriod[] {
  const seen = new Set<string>();
  const unique: AcademicPeriod[] = [];

  for (const period of periods) {
    const key = `${period.startDate.getTime()}-${period.endDate.getTime()}-${period.description}`;
    if (!seen.has(key)) {
      seen.add(key);
      unique.push(period);
    }
...
export function getVacations(structure: AcademicYearStructure): AcademicPeriod[] {
  ...
  return deduplicatePeriods(vacations);
}
...
export function getFreeDays(structure: AcademicYearStructure): AcademicPeriod[] {
  ...
  return deduplicatePeriods(freeDays);
}
