...
import {
  ...
  getUserTokens,
  getTokenStats
} from './calendar-subscription/calendar-token-manager';
import {
  ...
  UserEvent
} from './calendar-subscription/user-timetable-manager';
import {
  generateICalendar,
  generateICalendarForDateRange,
  validateICalendar,
  getCalendarMetadata
} from './calendar-subscription/icalendar-generator';
import {
  convertTimetableEntriesToEvents,
  getDefaultSemesterDates
} from './calendar-subscription/timetable-to-events-converter';
import {
  initializeMongoDB,
  getDatabase,
...
app.get('/field/:fieldId/year/:year/timetables', async (req: Request, res: Response) => {
  try {
    ...
    const timetables = groupTimetables.map((groupTimetable: any, groupIndex: number) => {
      const entries = groupTimetable.entries.map((entry: any, entryIndex: number) =>
...
app.post('/parse-multiple', async (req: Request, res: Response) => {
  try {
    ...
    const results = timetables.map((timetable: any) => ({
      ...timetable,
